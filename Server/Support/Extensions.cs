...
#pragma warning disable 8714 // The type cannot be used as type parameter in the generic type or method. Nullability of type argument doesn't match 'notnull' constraint.
    ...
    public static string GenerateTraceId(int length = 32)
    {
        ...
        var result = new char[length];
        for (int x = 0; x < length; x++)
            result[x] = idChars[Random.Shared.Next() % idChars.Length];
    ...
    /// <summary>
    /// Async support for <see cref="InvokeActionsWithProgressAsync(List{Action}, Action{Exception}?, CancellationToken?, ProgressChangedEventHandler?)"/>
    /// </summary>
    public static async Task InvokeAsyncActionsWithProgressAsync(List<Func<Task>> asyncActions, Action<Exception>? onException = null, CancellationToken? cancellationToken = null, ProgressChangedEventHandler? progressHandler = null)
    {
        if (asyncActions is null || asyncActions.Count == 0)
            return;
...
#pragma warning restore RECS0165 // Asynchronous methods should return a Task instead of void
    ...
    /// <param name="amount">Percentage to lighten. Value should be between 0 and 1.</param>
    /// <returns>Color</returns>
    public static Windows.UI.Color LighterBlue(this Windows.UI.Color color, float amount)
    {
